...
require_once '../classes/order_class.php';  // Assuming you have an Order class for managing orders
...
class CartController {
    ...
    public function __construct() {
        ...
        //$this->orderModel = new Order(); // Add this line to interact with the Order class
    }
    ...
    public function moveItemsToOrder($userId) {
        ...
        $orderDate = date('Y-m-d H:i:s'); // Get the current timestamp for order creation
        $status = 'pending'; // Default status before payment is processed
        ...
        // Optionally, clear the cart after moving items to the order
        return $this->clearCart($userId); // Clear the cart only if the items are successfully added to the order
    }
